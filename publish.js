...
function tutoriallink(tutorial, content) {
  if(!content){
    content = null;
  }
  return helper.toTutorial(tutorial, content, {
    tag: 'em',
    classname: 'disabled',
    prefix: 'Tutorial: '
  });
...
function buildPackageNav(packages, nav){
  var packageNames = Object.keys(packages);
  var buildNavItems = function(keys){
    return keys.sort().map(function(key){
      return '<li><a href="'+ key + '/latest">' + key + '</a></li>';
    }).join('');
  }
  var itemsByType = function(type){
    return packageNames.filter(function(value){ return value.indexOf(type) > -1; });
  }
  var armItems = itemsByType('-arm-');
  var asmItems = itemsByType('-asm-');
  var commonItems = itemsByType('-common');
  var serviceItems = Object.keys(packages).filter(function(item){
    return !(armItems.indexOf(item) > -1 || asmItems.indexOf(item) > -1 || commonItems.indexOf(item) > -1);
  }).sort();
  nav += '<h3>Azure SDK Packages</h3><ul>'
  nav += '<h3>Azure Services</h3><ul>' + buildNavItems(serviceItems) + '</ul>';
  nav += '<h3>Resource Management</h3><ul>' + buildNavItems(armItems) + '</ul>';
  nav += '<h3>Service Management</h3><ul>' + buildNavItems(asmItems) + '</ul>';
  nav += '<h3>Common Libs</h3><ul>' + buildNavItems(commonItems) + '</ul>';
  nav += '</ul>';
  return nav;
}

function buildTutorialNav(tutorials, linktoTutorial){
  var buildNavItem = function(item){
    var tutNav = '<li>' + linktoTutorial(item.longname, item.name.replace(/^module:/, ''));
    if(item.children && item.children.length > 0){
      tutNav += '<ul>';
      item.children.forEach(function(child){
        tutNav += buildNavItem(child);
      });
      tutNav += '</ul>';
    }
    tutNav += '</li>';
  ...
  itemsNav = '';
  if (tutorials.length) {
    itemsNav += '<h3>Tutorials</h3><ul>';
    tutorials.forEach(function(tutorial) {
      itemsNav += buildNavItem(tutorial);
    });
    itemsNav += '</ul>';
  }
  return itemsNav;
 ...
 */
function buildNav(members, packages, repoUrl, childPackage) {
  var home = "index.html";
  if(childPackage){
    home = "../../" + home;
  }
  ...
  if(repoUrl !== null && repoUrl !== undefined){
    var nav = '<h2><span><a href="' + home + '">Home</a> ' +
      '<span class="github-short">| <a href="' + repoUrl + '">GitHub</a></span></span></h2>';
  } else {
    var nav = '<h2><a href="' + home + '">Home</a></h2>';
  }
  var seen = {};
  var seenTutorials = {};
...
exports.publish = function(taffyData, opts, tutorials) {
  ...
  // once for all
  view.nav = buildNav(members, conf.packages, conf.repoUrl, conf.childPackage);
  ...
  function generateTutorial(title, tutorial, filename, parent) {
    var parentTitle = '';
    if(parent && parent.title){
      parentTitle = parent.title + ': ';
    }
    var tutorialData = {
      title: parentTitle + tutorial.title,
  ...
  function saveChildren(node) {
    node.children.forEach(function(child) {
      generateTutorial('Tutorial: ' + child.title, child, helper.tutorialToUrl(child.name), node);
      saveChildren(child);
    });
  }
